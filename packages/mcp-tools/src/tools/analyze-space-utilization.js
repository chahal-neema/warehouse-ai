...
async function analyzeSpaceUtilization(params) {
    const { areaId, aisle, includeRecommendations = true, minUtilization = 70, maxUtilization = 95 } = params;
    try {
        ...
        for (const [key, items] of groupedData) {
            ...
            const occupiedSlots = items.filter(item => item.qtyAvailUnits > 0 || item.qtyAvailEaches > 0).length;
            const utilizationPercent = totalCube > 0 ? Math.round((usedCube / totalCube) * 100) : 0;
            let efficiency;
            if (utilizationPercent < minUtilization)
                efficiency = 'Low';
            else if (utilizationPercent <= maxUtilization)
                efficiency = 'Optimal';
            else if (utilizationPercent <= 98)
                efficiency = 'High';
            else
                efficiency = 'Critical';
            utilization.push({
        ...
        const totalUtilizationPercent = totalCubeAcrossAll > 0
            ...
            : 0;
        let totalEfficiency;
        if (totalUtilizationPercent < minUtilization)
            totalEfficiency = 'Low';
        else if (totalUtilizationPercent <= maxUtilization)
            totalEfficiency = 'Optimal';
        else if (totalUtilizationPercent <= 98)
            totalEfficiency = 'High';
        else
            totalEfficiency = 'Critical';
        const totalStats = {
...
export const analyzeSpaceUtilizationTool = {
...
//# sourceMappingURL=analyze-space-utilization.js.map