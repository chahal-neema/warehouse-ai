...
async function findByLocation(params) {
    const { areaId, aisle, bay, levelNumber, zone, includeEmpty = false, sortBy = 'product' } = params;
    try {
        ...
        // Transform results
        const items = sortedResults.map(item => ({
            location: {
                areaId: item.areaId,
                aisle: item.aisle,
                bay: item.bay,
        ...
        if (aisle !== undefined)
            searchCriteria.aisle = aisle;
        if (bay !== undefined)
            searchCriteria.bay = bay;
        if (levelNumber !== undefined)
            searchCriteria.levelNumber = levelNumber;
        if (zone)
...
export const findByLocationTool = {
...
//# sourceMappingURL=find-by-location.js.map