...
async function inventoryStatusSearch(params) {
    const { inventoryStatus, slotStatus, palletStatus, areaId, includeMetrics = true, groupBy = 'none', limit = 500 } = params;
    try {
        ...
        const problemSlots = items.filter(item => item.slotStatus === 'DAMAGED' || item.slotStatus === 'BLOCKED' || item.slotStatus === 'MAINTENANCE');
        ...
        const palletIssues = items.filter(item => item.palletStatus === 'DAMAGED' || item.palletStatus === 'HOLD');
        ...
        if (inventoryStatus)
            searchCriteria.inventoryStatus = inventoryStatus;
        if (slotStatus)
            searchCriteria.slotStatus = slotStatus;
        if (palletStatus)
            searchCriteria.palletStatus = palletStatus;
        if (areaId)
            searchCriteria.areaId = areaId;
...
export const inventoryStatusSearchTool = {
...
//# sourceMappingURL=inventory-status-search.js.map