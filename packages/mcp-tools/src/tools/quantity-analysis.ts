...
interface QuantityAnalysisParams {
  analysisType: 'low_stock' | 'overstock' | 'zero_stock' | 'high_volume' | 'summary'
  minUnits?: number
  maxUnits?: number
  minEaches?: number
  maxEaches?: number
...
interface QuantityResult {
  ...
  quantityCategory: 'Zero' | 'Low' | 'Normal' | 'High' | 'Overstock'
  rackType: string
  slotType: string
}
...
interface QuantitySummary {
  ...
  averageEachesPerProduct: number
  quantityDistribution: Record<string, number>
  areaDistribution: Record<string, {
    products: number
    units: number
    eaches: number
    averageUnits: number
...
interface QuantityRecommendation {
  type: 'reorder' | 'redistribute' | 'remove_excess' | 'consolidate' | 'investigation'
...
async function quantityAnalysis(params: QuantityAnalysisParams): Promise<{
  items: QuantityResult[]
  ...
  try {
    ...
    const items: QuantityResult[] = results.map(item => {
      const totalQuantity = item.qtyAvailUnits + item.qtyAvailEaches
      let quantityCategory: QuantityResult['quantityCategory']
      
      if (totalQuantity === 0) quantityCategory = 'Zero'
      else if (totalQuantity <= 15) quantityCategory = 'Low'
      else if (totalQuantity <= 100) quantityCategory = 'Normal'
      else if (totalQuantity <= 500) quantityCategory = 'High'
      else quantityCategory = 'Overstock'
...
export const quantityAnalysisTool: MCPTool = {
