...
interface FindByPalletIdParams {
  palletId: string
...
async function findByPalletId(params: FindByPalletIdParams): Promise<{
  items: PalletResult[]
  totalFound: number
  palletId: string
  ...
  try {
    ...
    // Transform results
    const items: PalletResult[] = results.map(item => ({
...
export const findByPalletIdTool: MCPTool = {
