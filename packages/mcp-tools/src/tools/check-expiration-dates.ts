...
interface CheckExpirationDatesParams {
...
async function checkExpirationDates(params: CheckExpirationDatesParams): Promise<{
  ...
  try {
    ...
    const expiringItems: ExpiringItem[] = results
      .map(item => {
        const expirationDate = new Date(item.expirationDate!)
        const daysUntilExpiration = Math.ceil((expirationDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24))
        
        let priority: ExpiringItem['priority']
        if (daysUntilExpiration < 0) priority = 'Critical' // Already expired
        else if (daysUntilExpiration <= 7) priority = 'Critical'
        else if (daysUntilExpiration <= 14) priority = 'High'
        else if (daysUntilExpiration <= 21) priority = 'Medium'
        else priority = 'Low'
    ...
    if (expiringItems.length > 0) {
      summary.avgDaysUntilExpiration = Math.round(
        expiringItems.reduce((sum, item) => sum + item.daysUntilExpiration, 0) / expiringItems.length
      )
    }
    ...
    if (areaId) criteria.push(`Area: ${areaId.toUpperCase()}`)
...
export const checkExpirationDatesTool: MCPTool = {
