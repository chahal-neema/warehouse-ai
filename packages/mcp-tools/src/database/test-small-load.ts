...
async function testSmallLoad() {
  console.log('🧪 Testing small batch load (first 100 records)...')
  
  const filePath = '/mnt/c/Users/chaha/Cursor/warehouse-ai/USF_INVENTORY_ON_HAND_9B_2140_20250523_AM_EXTRACT.csv'
  ...
  return new Promise<void>((resolve, reject) => {
    fs.createReadStream(filePath)
      .pipe(csv(csvOptions))
      .on('data', (row: CSVRow) => {
        if (rowCount < 100) { // Only process first 100 records
          try {
      ...
      .on('end', async () => {
        try {
          console.log(`💾 Inserting ${records.length} test records...`)
          
          // Clear existing data first
          await prisma.inventoryLocation.deleteMany()
          
          // Try small batch insertions
          const batchSize = 5
          ...
          const productTest = await prisma.inventoryLocation.findFirst({
            where: { productNumber: "9375387" }
          })
          console.log('✅ Product query:', productTest?.prodDesc)
          
          const areaTest = await prisma.inventoryLocation.findMany({
            where: { areaId: "F" },
            take: 3
          })
          console.log(`✅ Frozen area query: ${areaTest.length} products found`)
